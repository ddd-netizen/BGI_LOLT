...
with col2:
    ...
    fig=px.pie(emp_stauts,values="Count", names="Employment Status", hole=0.5, width=1500, height=400)
    fig. update_traces(text=filtered_df["District"], textposition="inside")
...
expected_columns = ['Region', 'Year']  # Add more if needed for the bar chart
if all(col in df.columns for col in expected_columns):
    # Dropdowns for Region and Year
   # selected_region = st.selectbox("Select Region", sorted(df['Region'].dropna().unique()))
    ...
    # Create a bar chart
    # Assuming there's a column to use as x-axis and a numeric column for y-axis.
    # Replace 'Category' and 'Value' with real column names from your sheet.
    x_col = 'Category'  # <-- replace with actual column name for x-axis
    y_col = 'Value'     # <-- replace with actual column name for y-axis

    if x_col in filtered_df.columns and y_col in filtered_df.columns:
        fig = px.bar(filtered_df, x=x_col, y=y_col, title="Bar Chart")
        st.plotly_chart(fig)
    else:
        st.warning("Bar chart columns not found. Please check column names.")
else:
    st.error("Required columns 'Region' and 'Year' not found in the 'Anando' sheet.")
